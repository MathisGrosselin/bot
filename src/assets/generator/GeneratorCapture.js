...
import { OrderPromotion, Promotion, isPawn, kingSquares, pieceOrders, posV, triFusion } from "../Const";
            ...
            for (const ind of this.CaptureMove(i)) {
                ...
                if (isPawn(p) && (ind < 8 || ind > 55)) {
                    ...
                    if (lengthEnemyMove || isCheck) r.push({ i, ind, p: 10 - color, deleted: type, v: 800 });
                        ...
                        for (let a = 0; a < 4; a++)
                            r.push({ i, ind, p: Promotion[a] - color, deleted: type, v: OrderPromotion[a] });
                }
                //other Capture
                else
                    r.push({
