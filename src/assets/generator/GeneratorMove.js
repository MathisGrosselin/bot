        ...
        if (p && p % 2 === color)
            for (const ind of this.LegalMove(i)) {
                ...
                if (isPawn(p) && (ind < 8 || ind > 55)) {
                    ...
                    this.UnMakeMove();
                    if (lengthEnemyMove || isCheck) result.push({ i, ind, p: 10 - color, deleted: type, v: 800 });
                    else
                        for (let a = 0; a < 4; a++)
                        result.push({ i, ind, p: Promotion[a] - color, deleted: type, v: OrderPromotion[a] });
                } else {
                    let more = 0;
                    let t = ind;
                    let t2 = i;
